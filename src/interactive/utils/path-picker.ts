...
async function getLinuxVolumes(): Promise<Volume[]> {
	...
	if (lsblkOut) {
		try {
			...
			const flatten = (devices: LsblkDevice[]): LsblkDevice[] => devices.flatMap((d) => [d, ...(d.children ? flatten(d.children) : [])]);
			...
			for (const dev of flatten(data.blockdevices ?? [])) {
				...
				const name = dev.label ? `${dev.label} (${dev.name}) ${dev.size ?? ""}` : `${dev.name} ${dev.size ?? ""}`;
	...
	if (volumes.length === 0) {
		try {
			...
			for (const line of mounts.split("\n")) {
				...
				if (
					["tmpfs", "devtmpfs", "sysfs", "proc", "devpts", "cgroup", "cgroup2", "pstore", "bpf", "tracefs", "debugfs", "securityfs", "fusectl", "hugetlbfs", "mqueue", "autofs"].includes(
						fstype,
					)
				) continue;
				seen.add(mountpoint);
				const label = device.startsWith("/dev/") ? `💽 ${device.replace("/dev/", "")} → ${mountpoint}` : `🔗 ${mountpoint}`;
...
export class PathPicker {
	...
	private async navigate(dir: string, opts: { dirsOnly: boolean }): Promise<string> {
		...
		switch (chosen.kind) {
			case "action":
				return chosen.path;
			case "volume":
				return this.pickVolume().then((v) => this.navigate(v, opts));
			case "dir":
				return this.navigate(chosen.path, opts);
			case "file":
				return chosen.path;
		}
	}
	...
	private format(e: Entry): string {
		...
		const style = e.kind === "action" ? colors.brightGreen : e.kind === "volume" ? colors.brightMagenta : colors.brightBlue;
